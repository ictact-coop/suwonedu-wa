			...
			if(opts.show_panels) {
				for(i=j_frames.length-1;i>=0;i--) {
					...
					if(jf.find('.gv-panel-content').length>0) {
						...
						if(opts.ptyPoto=='Y'){
						alink = $('<a>');
						alink.attr('href',j_frames.eq(i).find('img').eq(0).attr('alt')).appendTo(p);
						}else if(opts.ptyPoto=='Z'){
						alink = $('<a rel="prettyPhoto[mixed]">');
						alink.attr('href',j_frames.eq(i).find('img').eq(0).attr('alt')).appendTo(p);
						}
						im = $('<img />');
						...
						if(jfimg.parent('a').length > 0){
							...
							if(opts.ptyPoto=='Y'||opts.ptyPoto=='Z') im.appendTo(alink);
							else im.appendTo(p);
						}
