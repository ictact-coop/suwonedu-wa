    ...
    class sejin7940_calendar extends WidgetHandler {
        ...
        function proc($args) {
			...
			if($args->date_var=='regdate') {
				...
				$obj->var_idx = substr($args->date_var,10);
				$obj->var_value_first = date('Ym', mktime(1,0,0,zdate($obj->regdate,'m'),1,zdate($obj->regdate,'Y')))."01";
				$obj->var_value_last = date('Ym',ztime($obj->regdate)).date('t', ztime($obj->regdate));
				$obj->sort_index = $obj->order_target = "regdate";
				$obj->order_type="desc";
				$obj->list_count="9999999";
                $output = executeQueryArray('widgets.sejin7940_calendar.getDocumentListWithExtraVars', $obj);
				if(count($output->data)) {
					foreach($output->data as $key => $val) {
						$oDocumentModel = &getModel('document'); 
						$getExtraVars = $oDocumentModel->getExtraVars($val->module_srl,$val->document_srl); 
						foreach($getExtraVars as $idx => $vals) {
							if($idx == $obj->var_idx) {
								$widget_info->calendar[$vals->value] = $val->title;
							}
						}
					}
